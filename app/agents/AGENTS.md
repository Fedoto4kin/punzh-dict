...
Не переклассифицирует. Вход: переводы леммы (`ArticleIndexTranslate`;
значения аддендумов уже в этом индексе) + закрытый список полей статьи
(имя, определение). Промпт и сбор переводов — из `dict.ai.prompts`
(общий с рантайм-`classify_article`). HTML и иллюстрации в промпт
...
Заливка: `load_translation_fields --file`. Боевой прогон — `backlog.md` §4
(очистка переводов на prod закрыта).
