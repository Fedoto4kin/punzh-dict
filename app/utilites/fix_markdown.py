...
def fix_puncts(article_html):
    ...
    for _ in re.finditer(r"\s([,;:\!\?\.]{1})", article_html):
        if _:
            article_html = article_html.replace(_.group(), _.group(1))
...
def split_i(article_html):
    ...
    for i in re.findall(r"<i>(?!\()(.*?)(?!\))</i>", article_html):
        if i and " " in i:
            article_html = article_html.replace(i, "</i> <i>".join(i.split()))
    return article_html
...
def move_dote_inside_i(article_html):
    ...
    if i:
        article_html = article_html.replace(i.group(), "<i>{}.</i>".format(i.group(1)))
...
def fix_whitespace_after_i(article_html):
    ...
    for _ in re.finditer(r"</i>([A-zA-я]+)", article_html):
        if _:
            ...
            article_html = article_html.replace(_.group(), "</i> {}".format(_.group(1)))
    return article_html
