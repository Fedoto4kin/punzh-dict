...
class SemanticField(models.Model):
    ...
    docs/method-onthlogy-markup.md). Используется веткой AI-поиска, НЕ основным поиском.
    """
...
class ArticleSemanticField(models.Model):
    ...
    from_translation — поле подтверждено переводами леммы, не смыслом
    из иллюстрации. Несколько True на статью нормальны (многозначность);
    все False — поля только из примеров или модель не уверена.
    """
    ...
    from_translation = models.BooleanField(
        default=False,
        verbose_name="По переводу",
    )
...
class ArticleKeyword(models.Model):
    ...
    См. docs/backlog-search.md (keywords в лексическом поиске).
