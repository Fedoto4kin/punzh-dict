...
- источники (`Source`);
- смысловые поля и ключевые слова (`SemanticField`, `ArticleSemanticField`, `ArticleKeyword`) — справочник и разметка для ветки AI-поиска; **не** смешивать с обычным `/search/`. Просмотр полей: `/ontology/` (закрытая бета, без пункта в шапке).
...
Отдельный просмотр поля: `/ontology/`, `/ontology/<id>/` (`search_by_semantic_field`). К размеченным статьям на лету добавляются неотмеченные отсылки «см. X» (входящий `ArticleLink`). Сортировка как у остальной выдачи (`sort_and_paginate`). Тесты: `dict/tests/test_ontology_view.py`. Админка: поля на статье и справочник — read-only.
